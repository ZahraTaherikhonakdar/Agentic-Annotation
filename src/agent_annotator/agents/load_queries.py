...
class LoadQueriesAgent(BaseAgent):
    ...
    def run(self, state: PipelineState, **_) -> PipelineState:
        ...
        # 4) Split filter / sample / preprocess
        df = self._filter_split(df)
        df = self._maybe_sample(df)
        df = self._preprocess(df)

        # 5) Normalize records and (optional) validate via schema
        ...
        # 6) Write TSV checkpoint (post-normalization)
        if self.output_tsv:
        ...
        # snapshot of config (for reproducibility)
        state["config"] = {
            "dataset_path": self.config.dataset_path,
            "split": self.config.split,
