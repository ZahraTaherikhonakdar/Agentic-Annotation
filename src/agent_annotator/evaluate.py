...
# Compare-to-human (original behavior)
JUDGE_SYSTEM_HUMAN = (
...
# -------------------------
# LLM client + JSON handling
# -------------------------
def _build_openai() -> OpenAI:
    key = os.getenv("OPENAI_API_KEY")
    if not key:
        raise RuntimeError("OPENAI_API_KEY not set")
    base = os.getenv("OPENAI_BASE_URL")  # e.g., https://api.deepseek.com/v1
    return OpenAI(api_key=key, base_url=base) if base else OpenAI(api_key=key)


def _use_json_mode() -> bool:
    """
    Use response_format JSON mode only when the backend supports it.
    We disable it for DeepSeek (which may not support OpenAI-style JSON mode).
    """
...
def _extract_json_obj(text: str) -> Dict[str, Any]:
    """
    Try to find a valid JSON object in arbitrary LLM output.
    Returns a parsed dict or {} if nothing valid is found.
    We keep the *last* valid object, which is often the final answer.
    """
    if not text:
        return {}
    last_good = None
...
def _judge_one_compare_to_human(
    client: OpenAI,
    model: str,
    query: str,
    human: str,
    ...
    """
    Judge compares AI label to human label.
    Uses strict JSON mode (response_format='json_object') just like the annotator.
    """
    user = (
        f"{JUDGE_TAXONOMY}\n"
        f'Query: "{query}"\n'
    ...
    try:
        resp = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": JUDGE_SYSTEM_HUMAN},
        ...
        # Debug raw if you want
        if os.getenv("DEBUG_LLM_JUDGE") == "1":
            print("[judge][raw]", txt)
        ...
        rat = str(data.get("rationale", "")).strip()[:120]
        ...
        # Any API / parsing error → fallback
        return {
            "decision": "incorrect",
            "confidence": 0.0,
...
def _judge_one_audit(
    ...
    """
    Judge classifies query itself and checks if AI label matches.
    Uses strict JSON mode like the annotator.
    """
    user = (
        f"{JUDGE_TAXONOMY}\n"
        f'Query: "{query}"\n'
    ...
    try:
        resp = client.chat.completions.create(
            model=model,
            messages=[
                {"role": "system", "content": JUDGE_SYSTEM_AUDIT},
                {"role": "user", "content": user},
            ],
            temperature=float(temperature),
            max_tokens=int(max_tokens),
